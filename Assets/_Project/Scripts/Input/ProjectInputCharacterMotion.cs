using System;
using _Project.Scripts.Systems.Platform;
using NeoFPS;
using NeoFPS.CharacterMotion;
using NeoFPS.CharacterMotion.Parameters;
using UnityEngine;

namespace _Project.Scripts.Input
{
    /// <summary>
    /// Project-side вариант NeoFPS InputCharacterMotion с явным разделением PC mouse-look и mobile touch-look.
    /// </summary>
    [HelpURL("https://docs.neofps.com/manual/inputref-mb-inputcharactermotion.html")]
    [RequireComponent(typeof(ICharacter))]
    public sealed class ProjectInputCharacterMotion : CharacterInputBase
    {
        [Header("Разделение платформ")]

        [SerializeField]
        [Tooltip("Запрещает чтение Mouse X / Mouse Y в мобильном режиме, чтобы touch WebGL не вращал камеру через mouse fallback.")]
        private bool m_BlockMouseLookOnMobile = true;

        [Header("Возможности")]

        [SerializeField]
        [Tooltip("Разрешить dodge по двойному нажатию направления движения.")]
        private bool m_EnableDodging = true;

        [SerializeField]
        [Range(0.1f, 1f)]
        [Tooltip("Максимальная пауза между двумя нажатиями направления для dodge.")]
        private float m_DodgeTimeout = 0.25f;

        [SerializeField]
        [Tooltip("Нужно ли заряжать прыжок удержанием кнопки вместо мгновенного прыжка.")]
        private bool m_EnableChargedJump = false;

        [SerializeField]
        [Range(0.1f, 51f)]
        [Tooltip("Время зарядки прыжка до полной силы, если включён charged jump.")]
        private float m_JumpChargeTime = 0.25f;

        [SerializeField]
        [Tooltip("Переключать lean нажатием или удерживать lean только пока нажата кнопка.")]
        private bool m_ToggleLean = true;

        [SerializeField]
        [Tooltip("Отменять sprint, если нет движения или аналоговый ввод слишком мал.")]
        private bool m_NoInputCancelsSprint = false;

        [Header("Параметры Motion Graph")]

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Trigger)]
        [Tooltip("Ключ trigger-параметра прыжка в motion graph персонажа.")]
        private string m_JumpKey = "jump";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Float)]
        [Tooltip("Ключ float-параметра зарядки прыжка в motion graph персонажа.")]
        private string m_JumpChargeKey = "jumpCharge";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Switch)]
        [Tooltip("Ключ switch-параметра удержания прыжка в motion graph персонажа.")]
        private string m_JumpHoldKey = "jumpHold";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Switch)]
        [Tooltip("Ключ switch-параметра crouch в motion graph персонажа.")]
        private string m_CrouchKey = "crouch";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Switch)]
        [Tooltip("Ключ switch-параметра удержания crouch в motion graph персонажа.")]
        private string m_CrouchHoldKey = "crouchHold";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Switch)]
        [Tooltip("Ключ switch-параметра sprint в motion graph персонажа.")]
        private string m_SprintKey = "sprint";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Switch)]
        [Tooltip("Ключ switch-параметра удержания sprint в motion graph персонажа.")]
        private string m_SprintHoldKey = "sprintHold";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Trigger)]
        [Tooltip("Ключ trigger-параметра dodge влево в motion graph персонажа.")]
        private string m_DodgeLeftKey = "dodgeLeft";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Trigger)]
        [Tooltip("Ключ trigger-параметра dodge вправо в motion graph персонажа.")]
        private string m_DodgeRightKey = "dodgeRight";

        [SerializeField]
        [MotionGraphParameterKey(MotionGraphParameterType.Trigger)]
        [Tooltip("Ключ trigger-параметра ability в motion graph персонажа.")]
        private string m_AbilityKey = "ability";

        [Header("Дополнительные input-привязки")]

        [SerializeField]
        [Tooltip("Дополнительные кнопки, которые напрямую управляют параметрами motion graph.")]
        private MotionGraphInput[] m_CustomInputs = { };

        private CharacterInteractionHandler m_InteractionMgr = null;
        private MouseAndGamepadAimController m_Aimer = null;
        private BodyLeanBase m_BodyLean = null;

        private bool m_Initialised = false;

        private TriggerParameter m_JumpTrigger = null;
        private FloatParameter m_JumpChargeProperty = null;
        private SwitchParameter m_JumpHoldProperty = null;
        private SwitchParameter m_CrouchProperty = null;
        private SwitchParameter m_CrouchHoldProperty = null;
        private SwitchParameter m_SprintProperty = null;
        private SwitchParameter m_SprintHoldProperty = null;
        private TriggerParameter m_DodgeLeftTrigger = null;
        private TriggerParameter m_DodgeRightTrigger = null;
        private TriggerParameter m_AbilityTrigger = null;

        private float m_DodgeLeftTimer = 0f;
        private float m_DodgeRightTimer = 0f;

        [Serializable]
        private struct MotionGraphInput
        {
            [Tooltip("Ключ параметра motion graph.")]
            public string parameterKey;

            [Tooltip("Тип параметра motion graph.")]
            public ParameterType type;

            [Tooltip("Кнопка NeoFPS, которая управляет параметром.")]
            public FpsInputButton button;

            public MotionGraphParameter parameter { get; private set; }

            public void Initialise(MotionGraphContainer motionGraph)
            {
                switch (type)
                {
                    case ParameterType.Trigger:
                        parameter = motionGraph.GetTriggerProperty(parameterKey);
                        break;
                    case ParameterType.Switch:
                        parameter = motionGraph.GetSwitchProperty(parameterKey);
                        break;
                }
            }
        }

        private enum ParameterType
        {
            Trigger,
            Switch
        }

        protected override void OnAwake()
        {
            base.OnAwake();

            m_InteractionMgr = GetComponent<CharacterInteractionHandler>();
            m_Aimer = GetComponent<MouseAndGamepadAimController>();

            if (m_Character.bodyTransformHandler != null)
            {
                m_BodyLean = m_Character.bodyTransformHandler.GetComponent<BodyLeanBase>();
            }
        }

        protected override void OnGainFocus()
        {
            base.OnGainFocus();

            NeoFpsInputManagerBase.captureMouseCursor = true;
        }

        protected override void OnLoseFocus()
        {
            m_Character.motionController.inputMoveDirection = Vector2.zero;
            m_Character.motionController.inputMoveScale = 0f;

            if (m_JumpHoldProperty != null)
            {
                m_JumpHoldProperty.Hold(false);
            }

            if (m_SprintHoldProperty != null)
            {
                m_SprintHoldProperty.Hold(false);
            }

            if (m_CrouchHoldProperty != null)
            {
                m_CrouchHoldProperty.Hold(false);
            }

            NeoFpsInputManagerBase.captureMouseCursor = false;
        }

        protected override void UpdateInput()
        {
            CheckMotionGraphConnection();
            UpdateLookInput();
            UpdateMoveInput();
            UpdateInteractionInput();
        }

        private void CheckMotionGraphConnection()
        {
            if (m_Initialised)
            {
                return;
            }

            MotionGraphContainer motionGraph = m_Character.motionController.motionGraph;
            m_JumpTrigger = motionGraph.GetTriggerProperty(m_JumpKey);
            m_JumpChargeProperty = motionGraph.GetFloatProperty(m_JumpChargeKey);
            m_JumpHoldProperty = motionGraph.GetSwitchProperty(m_JumpHoldKey);
            m_CrouchProperty = motionGraph.GetSwitchProperty(m_CrouchKey);
            m_CrouchHoldProperty = motionGraph.GetSwitchProperty(m_CrouchHoldKey);
            m_SprintProperty = motionGraph.GetSwitchProperty(m_SprintKey);
            m_SprintHoldProperty = motionGraph.GetSwitchProperty(m_SprintHoldKey);
            m_DodgeLeftTrigger = motionGraph.GetTriggerProperty(m_DodgeLeftKey);
            m_DodgeRightTrigger = motionGraph.GetTriggerProperty(m_DodgeRightKey);
            m_AbilityTrigger = motionGraph.GetTriggerProperty(m_AbilityKey);

            for (int i = 0; i < m_CustomInputs.Length; ++i)
            {
                m_CustomInputs[i].Initialise(m_Character.motionController.motionGraph);
            }

            m_Initialised = true;
        }

        private void UpdateLookInput()
        {
            if (!allowLookInput || m_Aimer == null)
            {
                return;
            }

            Vector2 mouseLook = ShouldBlockMouseLook()
                ? Vector2.zero
                : new Vector2(GetAxis(FpsInputAxis.MouseX), GetAxis(FpsInputAxis.MouseY));

            m_Aimer.HandleMouseInput(mouseLook);
            m_Aimer.HandleAnalogInput(new Vector2(
                GetAxis(FpsInputAxis.LookX),
                GetAxis(FpsInputAxis.LookY)
            ));
        }

        private bool ShouldBlockMouseLook()
        {
            return m_BlockMouseLookOnMobile && ProjectPlatformProvider.IsMobile;
        }

        private void UpdateMoveInput()
        {
            if (!allowMoveInput)
            {
                return;
            }

            Vector2 move = new Vector2(
                GetAxis(FpsInputAxis.MoveX),
                GetAxis(FpsInputAxis.MoveY)
            );

            if (GetButton(FpsInputButton.Forward))
            {
                move.y += 1f;
            }

            if (GetButton(FpsInputButton.Backward))
            {
                move.y -= 1f;
            }

            if (GetButton(FpsInputButton.Left))
            {
                move.x -= 1f;
            }

            if (GetButton(FpsInputButton.Right))
            {
                move.x += 1f;
            }

            float mag = Mathf.Clamp01(move.magnitude);
            if (mag > 0.001f)
            {
                move.Normalize();
            }

            m_Character.motionController.inputMoveDirection = move;
            m_Character.motionController.inputMoveScale = mag;

            UpdateMovementModifiers(mag);
            UpdateJumpInput();
            UpdateDodgeInput();
            UpdateAbilityInput();
            UpdateLeanInput();
            UpdateCustomMotionGraphInputs();
        }

        private void UpdateMovementModifiers(float moveMagnitude)
        {
            bool sprintPress = GetButtonDown(FpsInputButton.SprintToggle);
            bool sprintHold = GetButton(FpsInputButton.Sprint);
            bool crouchPress = GetButtonDown(FpsInputButton.CrouchToggle);
            bool crouchHold = GetButton(FpsInputButton.Crouch);

            if (m_SprintProperty != null)
            {
                if (m_NoInputCancelsSprint && moveMagnitude < 0.1f)
                {
                    sprintPress = false;
                    m_SprintProperty.on = false;
                }

                m_SprintProperty.SetInput(sprintPress, sprintHold);

                if (crouchPress || crouchHold)
                {
                    m_SprintProperty.on = false;
                }
            }

            if (m_SprintHoldProperty != null)
            {
                m_SprintHoldProperty.on = sprintHold || GetButton(FpsInputButton.SprintToggle);
            }

            if (m_CrouchProperty != null)
            {
                m_CrouchProperty.SetInput(crouchPress, crouchHold);

                if (sprintPress || sprintHold)
                {
                    m_CrouchProperty.on = false;
                }
            }

            if (m_CrouchHoldProperty != null)
            {
                m_CrouchHoldProperty.Hold(crouchHold || GetButton(FpsInputButton.CrouchToggle));
            }
        }

        private void UpdateJumpInput()
        {
            if (m_JumpTrigger == null)
            {
                return;
            }

            if (m_EnableChargedJump && m_JumpChargeProperty != null)
            {
                if (GetButtonDown(FpsInputButton.Jump))
                {
                    m_JumpChargeProperty.value = 0f;
                }

                if (GetButton(FpsInputButton.Jump))
                {
                    m_JumpChargeProperty.value = Mathf.Clamp01(m_JumpChargeProperty.value + Time.deltaTime / m_JumpChargeTime);
                }

                if (GetButtonUp(FpsInputButton.Jump))
                {
                    m_JumpTrigger.Trigger();
                }
            }
            else if (GetButtonDown(FpsInputButton.Jump))
            {
                m_JumpTrigger.Trigger();
            }

            if (m_JumpHoldProperty != null)
            {
                m_JumpHoldProperty.Hold(GetButton(FpsInputButton.Jump));
            }
        }

        private void UpdateDodgeInput()
        {
            if (!m_EnableDodging || m_DodgeLeftTrigger == null || m_DodgeRightTrigger == null)
            {
                return;
            }

            if (GetButtonDown(FpsInputButton.Left))
            {
                if (m_DodgeLeftTimer > 0.001f)
                {
                    m_DodgeLeftTrigger.Trigger();
                }
                else
                {
                    m_DodgeLeftTimer = m_DodgeTimeout;
                }
            }

            if (GetButtonDown(FpsInputButton.Right))
            {
                if (m_DodgeRightTimer > 0.001f)
                {
                    m_DodgeRightTrigger.Trigger();
                }
                else
                {
                    m_DodgeRightTimer = m_DodgeTimeout;
                }
            }

            m_DodgeLeftTimer = Mathf.Max(0f, m_DodgeLeftTimer - Time.deltaTime);
            m_DodgeRightTimer = Mathf.Max(0f, m_DodgeRightTimer - Time.deltaTime);
        }

        private void UpdateAbilityInput()
        {
            if (m_AbilityTrigger != null && GetButtonDown(FpsInputButton.Ability))
            {
                m_AbilityTrigger.Trigger();
            }
        }

        private void UpdateLeanInput()
        {
            if (m_BodyLean == null)
            {
                return;
            }

            if (m_ToggleLean)
            {
                UpdateToggleLeanInput();
            }
            else
            {
                UpdateHoldLeanInput();
            }
        }

        private void UpdateToggleLeanInput()
        {
            if (GetButtonDown(FpsInputButton.LeanLeft))
            {
                if (Mathf.Abs(m_BodyLean.targetLean) > 0.9f)
                {
                    m_BodyLean.ResetLean();
                }
                else
                {
                    m_BodyLean.LeanLeft(1f);
                }
            }

            if (GetButtonDown(FpsInputButton.LeanRight))
            {
                if (Mathf.Abs(m_BodyLean.targetLean) > 0.9f)
                {
                    m_BodyLean.ResetLean();
                }
                else
                {
                    m_BodyLean.LeanRight(1f);
                }
            }
        }

        private void UpdateHoldLeanInput()
        {
            int lean = 0;
            if (GetButton(FpsInputButton.LeanLeft))
            {
                --lean;
            }

            if (GetButton(FpsInputButton.LeanRight))
            {
                ++lean;
            }

            switch (lean)
            {
                case -1:
                    m_BodyLean.LeanLeft(1f);
                    break;
                case 1:
                    m_BodyLean.LeanRight(1f);
                    break;
                case 0:
                    m_BodyLean.ResetLean();
                    break;
            }
        }

        private void UpdateCustomMotionGraphInputs()
        {
            for (int i = 0; i < m_CustomInputs.Length; ++i)
            {
                if (m_CustomInputs[i].button == FpsInputButton.None || string.IsNullOrWhiteSpace(m_CustomInputs[i].parameterKey))
                {
                    continue;
                }

                switch (m_CustomInputs[i].type)
                {
                    case ParameterType.Trigger:
                    {
                        var parameter = m_CustomInputs[i].parameter as TriggerParameter;
                        if (parameter != null && GetButtonDown(m_CustomInputs[i].button))
                        {
                            parameter.Trigger();
                        }

                        break;
                    }
                    case ParameterType.Switch:
                    {
                        var parameter = m_CustomInputs[i].parameter as SwitchParameter;
                        if (parameter != null)
                        {
                            parameter.on = GetButton(m_CustomInputs[i].button);
                        }

                        break;
                    }
                }
            }
        }

        private void UpdateInteractionInput()
        {
            if (m_InteractionMgr == null || !m_InteractionMgr.enabled)
            {
                return;
            }

            if (GetButtonDown(FpsInputButton.Use))
            {
                m_InteractionMgr.InteractPress();
            }

            if (GetButtonUp(FpsInputButton.Use))
            {
                m_InteractionMgr.InteractRelease();
            }
        }
    }
}
